...
import React, { useState, useEffect } from 'react';
import { View, Text } from 'react-native';
import { Api } from '../Services/Api'; // Import the Api function from Api.js

export default function HomeScreen() {
  const [seriesId, setSeriesId] = useState('tt0412142'); // Set the initial series ID
  const [seriesData, setSeriesData] = useState(null); // State variable to hold series data

  useEffect(() => {
    const fetchSeriesData = async () => {
      const response = await Api(seriesId);
      setSeriesData(response);
    };

    fetchSeriesData();
  }, [seriesId]); // Update the API call when the seriesId changes

  return (
    <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
      {seriesData && (
        <View>
          <Text>Series Title: {seriesData.title}</Text>
          <Text>Series Original Title: {seriesData.originalTitle}</Text>
          <Image source={{ uri: seriesData.image.url }} style={{ width: 100, height: 100 }} />
        </View>
      )}
    </View>
  );
